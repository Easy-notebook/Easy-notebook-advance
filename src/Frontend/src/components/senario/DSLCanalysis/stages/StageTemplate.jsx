...
// ==================== Generic Stage Component ====================
const StageTemplate = ({ config, onComplete }) => {
    const { stageId, steps, initialVariables, initialChecklist } = config;
    ...
    useEffect(() => {
        if (stageIdRef.current !== stageId) {
            ...
            if (initialChecklist && !useAIPlanningContextStore.getState().isStageComplete(stageId)) {
                setChecklist(initialChecklist.todo || [], initialChecklist.done || []);
            }
            return;
        }
        ...
        if (stepsChanged) {
            ...
            if (currentStepIndex < steps.length &&
                currentStepRef.current !== steps[currentStepIndex].stepId) {
                debouncedSwitchStep(steps[currentStepIndex].stepId);
            }
        }
    }, [stageId, steps, debouncedSwitchStep, clearStep, currentStepIndex, addVariable, setChecklist, initialVariables, initialChecklist]);
    ...
    const loadStep = useCallback(async (stepIndex) => {
        ...
        setError(null);
        setErrorDetails(null);
        const stepId = currentSteps[stepIndex].stepId;
        ...
        try {
            ...
            if (err.name === 'AbortError') {
                ...
                const stepTitle = currentSteps[stepIndex].title || `Step ${stepIndex + 1}`;
    ...
    }, [streamCompleted, autoAdvance, currentStepIndex, loadStep, markStageAsComplete, stageId]);

    useEffect(() => {
        if (isStageComplete && !historyLoaded) {
            const currentSteps = stepsRef.current;
            const lastStep = currentSteps[currentSteps.length - 1];
            debouncedSwitchStep(lastStep.stepId);
    ...
    }, [stepsLoaded.length, currentStepIndex, loadStep, addVariable, toDoList, isStageComplete, stageId, initialVariables, variables]);
                                ...
                                <div className="overflow-y-auto">
                                    {stepsLoaded.includes(currentStepIndex) && currentStepIndex < currentSteps.length ? (
                                        <PersistentStepContainer
                                            ...
                                            stepId={currentSteps[currentStepIndex].stepId}
                                        />
                                    ) : (
                                        <EmptyState />
                                    )}
